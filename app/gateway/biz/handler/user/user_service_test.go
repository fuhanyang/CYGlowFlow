...
func TestUploadAvatar(t *testing.T) {
	h := server.Default()
	h.POST("/user/upload-avatar", UploadAvatar)
	path := "/user/upload-avatar"                             // todo: you can customize query
	body := &ut.Body{Body: bytes.NewBufferString(""), Len: 1} // todo: you can customize body
	header := ut.Header{}                                     // todo: you can customize header
...
func TestUpdateUserInfo(t *testing.T) {
	h := server.Default()
	h.PUT("/user/info", UpdateUserInfo)
	path := "/user/info"                                      // todo: you can customize query
	body := &ut.Body{Body: bytes.NewBufferString(""), Len: 1} // todo: you can customize body
	header := ut.Header{}                                     // todo: you can customize header
...
func TestSearchUser(t *testing.T) {
	h := server.Default()
	h.GET("/user/search", SearchUser)
	path := "/user/search"                                    // todo: you can customize query
	body := &ut.Body{Body: bytes.NewBufferString(""), Len: 1} // todo: you can customize body
	header := ut.Header{}                                     // todo: you can customize header
	w := ut.PerformRequest(h.Engine, "GET", path, body, header)
	resp := w.Result()
